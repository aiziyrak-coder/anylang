#!/usr/bin/env python3
"""Deploy friends avatar_url (business logo) fix."""
...
FILES = ["backend/app/services/friends.py"]
...
def main() -> int:
    ...
    print("\n".join(ln for ln in err.splitlines() if "password" not in ln.lower())[-600:])
    code = o.channel.recv_exit_status()
    _, o2, _ = c.exec_command(
        "sleep 6; curl -sS -m 10 http://127.0.0.1:8105/health",
        timeout=30,
    )
